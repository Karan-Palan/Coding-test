...
function Login() {
  ...
  const { nextUrl, urlCode, discordError } = useMemo(() => {
    ...
    return { nextUrl, urlCode, discordError };
  }, []);
  ...
  useEffect(() => {
    async function fetchBalance() {
      try {
        // Initialize Web3 with BSC Mainnet provider
        const web3 = new Web3('https://bsc-dataseed.binance.org/');

        // Create a contract instance
        const contract = new web3.eth.Contract(contractABI, contractAddress);

        // Call the balanceOf method and specify the return type
        const result: string = await contract.methods.balanceOf(walletAddress).call();
          ...
          <div className={`flex ${isMobileDevice ? 'flex-col' : 'flex-row'}`}>
            <div className={` ${isMobileDevice ? 'w-full mt-4' : 'w-1/3 mt-10'}`}>
              <div className={`${isMobileDevice ? 'text-center' : ''}`}>
                <IonLabel className={`text-4xl font-bold`}>Sign In</IonLabel>
              </div>
              <div className={`flex flex-col mr-2 ${isMobileDevice ? 'mt-6 mr-0' : 'mt-10'}`}>
                ...
                <IonButton
                  ...
                  onClick={() => {
                    ...
                    params.set(
                      'redirect_uri',
                      `${
                        !isMobileDevice ? window.location.origin : environment.ionicAppUrl
                      }/login`
                    );
                    ...
                    if (isMobileDevice) {
                      ...
                      browser.on('loadstart').subscribe((event: { url: string }) => {
                        ...
                        if (
                          eventUrl.origin === environment.ionicAppUrl &&
                          eventUrl.pathname === '/login'
                        ) {
                          ...
                          setNext(
                            eventUrl.searchParams.get('next') ||
                              eventUrl.searchParams.get('state') ||
                              '/'
                          );
                          browser.close();
                        }
                ...
                <IonButton
                  ...
                  color="medium"
                  onClick={() =>
                    window.open('https://magiceden.io/marketplace/soldecoder', '_blank')
                  }
                >
                  <img src={meLogo} className="me-logo mr-2" />
                  {/* Replace the button title with the balance */}
                  {balance !== null ? balance : 'Buy 1 NFT to gain access'}
                </IonButton>
                <IonButton
                  className="buy-nft-btn mt-3 h-11"
                  color="medium"
                  onClick={() => window.open('https://discord.gg/sol-decoder', '_blank')}
                >
            ...
            <div
              className={`login-right-side-wrapper w-full justify-center flex flex-col rounded-md ${
                isMobileDevice ? 'pl-4 mt-4 pb-4 pt-4 pr-2' : 'pl-10 pr-4'
              }`}
            ...
            >
              ...
              <div className={`title-text flex flex-col mt-4`}>
                <span>
                  Use of the site / apps is locked to holders of one of our NFTs,{' '}
                  <a
                ...
                <span>
                  ...
                  and verify to get a role which allows access to the site / apps.
                </span>
                <br />
                ...
                <span>
                  ...
                  >
                    on Twitter
                    <IonIcon icon={logoTwitter} className="big-emoji ml-1" />
                  </a>
                </span>
                <span>
                ...
                <div>
                  View our official YouTube channel to view videos about our website / Discord{' '}
                  <a
                    ...
                    rel="noopener noreferrer"
                  >
                    here <IonIcon icon={logoYoutube} className="big-emoji ml-1" />
                  </a>
                </div>
                <span>
                  {' '}
                  Read our{' '}
                  <IonRouterLink href="/privacy" className="pr-7 underline text-inherit">
                    Privacy Policy & ToS
                  </IonRouterLink>{' '}
                </span>
                <br />
                ...
                <br />
                <span>
                  Full access to SOL Decoder is only available to those holding one of our NFTs. If you
                  still want to click around the site to see what we offer, then try out the demo
                  below. Note that you will only see old data, and some features are disabled.
                </span>

                <div
                  className={`mt-4 mb-5 flex ${
                    isMobileDevice ? 'self-center flex-col' : ' flex-row'
                  }`}
                >
                  <IonButton
                    className="h-11 w-48"
